...
*Setup instructions for a gitian build of CTSC using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CTSC
...
Clone the git repositories for ctsc and gitian and then checkout the ctsc version that you want to build.
...
**Note**: if you get the error message *"No such script: /usr/share/debootstrap/scripts/trusty"* you have to add a symlink for trusty, the same as other ubuntu releases in /usr/share/debootstrap/scripts:
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ctsc repository
...
To build CTSC (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ctsc repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ctsc/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ctscoin/ctsc-core
...
For example:
```bash
URL=https://github.com/konez2k/ctsc.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ctsc=${COMMIT} --url ctsc=${URL} ../ctsc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ctsc=${COMMIT} --url ctsc=${URL} ../ctsc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ctsc=${COMMIT} --url ctsc=${URL} ../ctsc/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ctsc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ctsc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ctsc-build.assert
```
...
[ctsc/gitian.sigs](https://github.com/ctsc/gitian.sigs/) repository, or if that's not possible to create a pull
request.
